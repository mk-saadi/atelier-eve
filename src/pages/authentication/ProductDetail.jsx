...
const ProductDetail = () => {
	...
	useEffect(() => {
		try {
			const sizes = product?.clothSize.split(",");
			setSize(sizes);
		} catch (error) {
			console.error("Error splitting sizes:", error);
		}
	}, [product.clothSize]);
			...
			<div className="grid grid-cols-1 overflow-x-hidden lg:grid-cols-7 lg:gap-x-2 gap-y-4 lg:gap-y-0">
				...
				<div className="col-span-2 mt-8">
					<form onSubmit={handleSubmit}>
						...
						<p className="text-2xl font-bold tracking-tight text-gray-900/80">
							$ {product.price}
						</p>
									...
									{color.map((hex, index) => (
										<RadioGroup.Option
											...
											className={`relative  flex cursor-pointer items-center justify-center rounded-full focus:outline-none ring-2 ring-gray-400 ring-offset-2 ${
												selectedColor === hex
													? "ring-2 ring-orange-400 ring-offset-2 scale-90"
													: ""
											}`}
										>
											<span
